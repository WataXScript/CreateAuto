...
if not safeIsFolder(FS_BASE) and hasMakeFolder then
    safeMakeFolder(FS_BASE)
end
...
function FileSystem:listFolders(base)
    ...
    if safeIsFolder(base) then
        local files = safeListFiles(base)
        if files then
            for _,f in ipairs(files) do
                if safeIsFolder(f) then
...
function FileSystem:listFiles(folder)
    folder = folder or ""
    local basePath = joinPath(FS_BASE, folder)
    local out = {}

    if safeIsFolder(basePath) then
        local files = safeListFiles(basePath)
        if files then
            for _,f in ipairs(files) do
                local name = f:match("([^/\\]+)$") or f
    ...
    local root = workspace:FindFirstChild(FS_BASE)
    if root then
        local node = folder ~= "" and root:FindFirstChild(folder) or root
        if node then
...
function FileSystem:readFileFull(path)
    local fullPath = joinPath(FS_BASE, path)
    local content = safeReadFile(fullPath)
