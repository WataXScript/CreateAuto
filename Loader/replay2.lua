...
local function loadReplayJSON(fileName)
    local path = ReplayFolder.."/"..fileName
...
local function playMultiReplays(framesList)
    ...
    while isRunning and #pool > 1 do
        
        local idx, dist = 1, math.huge
        local pos = hrp.Position
        for i,cf in ipairs(pool) do
            local d = (cf.Position - pos).Magnitude
            if d < dist then
                dist = d
                idx = i
...
local function showReplaySelectorSingle()
    local files = getReplayFiles()
    if #files == 0 then
        warn("Tidak ada replay JSON di folder", ReplayFolder)
        return
    end

    local Popup = Instance.new("Frame", screenGui)
    Popup.Size = UDim2.new(0, 200, 0, #files*30)
    Popup.Position = UDim2.new(0.5, -100, 0.5, -(#files*15))
    ...
    for i, name in ipairs(files) do
        ...
        Btn.MouseButton1Click:Connect(function()
            Popup:Destroy()
            local frames = loadReplayJSON(name)
            playReplay(frames)
        end)
    end
end
...
local function showReplaySelectorMulti()
    local files = getReplayFiles()
    if #files == 0 then
        warn("Tidak ada replay JSON di folder", ReplayFolder)
        return
    ...
    local Popup = Instance.new("Frame", screenGui)
    Popup.Size = UDim2.new(0, 220, 0, (#files*30)+40)
    Popup.Position = UDim2.new(0.5, -110, 0.5, -(#files*15))
    Popup.BackgroundColor3 = Color3.fromRGB(30,30,30)
    Instance.new("UICorner", Popup).CornerRadius = UDim.new(0,8)
    ...
    okBtn.Size = UDim2.new(0.5,-5,0,30)
    okBtn.Position = UDim2.new(0,5,1,-35)
    okBtn.Text = "OK"
    ...
    cancelBtn.Size = UDim2.new(0.5,-5,0,30)
